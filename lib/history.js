...
const send = msg => receiver => receiver[msg]()
const destroyEntry = send("destroy")

module.exports = class History {
  serialize() {
    const isValid = entry => entry.isValid()
    const serialize = entry => entry.serialize()
    return {
      index: this.index,
      entries: this.entries.filter(isValid).map(serialize),
    }
  }
  ...
  static deserialize(createLocation, state) {
    ...
    return Object.assign(new this(createLocation), {
      index: state.index,
      entries: state.entries.map(complementEditor).map(Entry.deserialize),
    })
  }
  ...
  //    No special @index adjustment.
  //
  add(location, {subject, setIndexToHead = true} = {}) {
    let entry
  ...
  removeInvalidEntries() {
    // Scrub invalid
    const destroy = entry => entry.destroy()
    const isValid = entry => entry.isValid()
    const isNotValid = entry => !entry.isValid()

    this.entries.filter(isNotValid).forEach(destroy)
    this.entries = this.entries.filter(isValid)

    // Remove if exceeds max
    const removeCount = this.entries.length - atom.config.get("cursor-history.max")
    if (removeCount > 0) {
      this.entries.splice(0, removeCount).forEach(destroy)
    }
  }

  inspect(msg) {
    return this.entries
      .map((e, i) => `${i === this.index ? "> " : "  "}${i}: ${e.inspect()}`)
      .join("\n")
  }
  ...
  jump(editor, direction, {withinEditor} = {}) {
    ...
    if (direction === "prev" && wasAtHead) {
      ...
      this.add(location, {setIndexToHead: false, subject: "Save head position"})
      needToLog = false
    }

    if (editor.getURI() === URI) {
      this.land(editor, point, direction, {log: needToLog})
      return
    }
    ...
    if (item) {
      ...
      this.land(item, point, direction, {forceFlash: true, log: needToLog})
      return
    }

    const searchAllPanes = atom.config.get("cursor-history.searchAllPanes")
    atom.workspace.open(URI, {searchAllPanes}).then(editor => {
      this.land(editor, point, direction, {forceFlash: true, log: needToLog})
    })
  }

  land(editor, point, direction, {forceFlash, log} = {}) {
    const originalRow = editor.getCursorBufferPosition().row
    editor.setCursorBufferPosition(point, {autoscroll: false})
    editor.scrollToCursorPosition({center: true})

    if (atom.config.get("cursor-history.flashOnLand")) {
      if (forceFlash || originalRow !== point.row) this.flash(editor)
    }
    if (atom.config.get("cursor-history.debug") && log) this.log(direction)
  }
