...
module.exports = class History {
  ...
  setIndexToHead () {
    // Since we know we can set index to entries.length, we can safely remove invalid entries here.
    // Scrub invalid
  ...
  add (location, {setIndexToHead = true} = {}) {
    ...
    if (getConfig('debug') && location.reason) {
  ...
  jump (editor, direction, editorToFind) {
    ...
    // NOTE indexIsAtHead() check must be done BEFORE updating index on next line
    const needToSaveLocation = direction === 'prev' && this.indexIsAtHead()

    this.index = index
    const entry = this.entries[index]

    if (needToSaveLocation) {
      const location = {
        editor,
    ...
    if (entry.editor && !entry.URI) {
      ...
      this.land(entry.editor, entry.point, direction, true)
    } else {
      const options = {searchAllPanes: getConfig('searchAllPanes')}
      atom.workspace.open(entry.URI, options).then(editor => {
        this.land(editor, entry)
        if (getConfig('debug')) {
          this.log(direction)
        }
      })
  ...
  land (editor, entry) {
    ...
    const needFlash = getConfig('flashOnLand') && (editor.getURI() !== entry.URI || oldRow !== point.row)
    if (needFlash) {
      this.flash(editor)
    }
  }
  ...
  flash (editor) {
    ...
    this.flashTimeout = setTimeout(this.destroyFlash, 1000)
  }
