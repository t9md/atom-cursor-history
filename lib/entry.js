...
const editorByURI = new WeakMap()
function itemForURI (uri) {
  if (!editorByURI.has(uri)) {
    const pane = atom.workspace.paneForItem(uri)
    editorByURI.set(uri, pane ? pane.itemForURI(uri) : undefined)
  }
  return editorByURI.get(uri)
}
...
module.exports = class Entry {
  ...
  inspect () {
    if (this.destroyed) {
      ...
      return `${this.point}, ${basename(this.URI)}` + invalid
    }
  }
}
