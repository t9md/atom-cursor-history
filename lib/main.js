...
class Location {
  ...
  needToSave (editor, rowDelta, columnDelta) {
...
module.exports = {
  ...
  checkLocationChange (location, timeout) {
    ...
    return setTimeout(() => {
      ...
      if (editor) {
        const reason = location.needToSave(editor, this.rowDeltaToRemember, this.columnDeltaToRemember)
        if (reason) {
          this.getHistory().add(location, {message: `${reason} [${location.command}]`})
        }
      }
    }, timeout)
  }
